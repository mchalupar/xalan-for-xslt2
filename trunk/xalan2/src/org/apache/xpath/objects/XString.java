...
public class XString extends XObject implements XMLString
{
  ...
  public void callVisitors(ExpressionOwner owner, XPathVisitor visitor)
  {
	...
	@Override
	public boolean endsWith(XMLString suffix) {
		
		return endsWith(suffix,0);
	}

	@Override
	public boolean endsWith(XMLString suffix, int toffset) {
		int to = toffset;
	    int tlim = this.length();
	    int po = 0;
	    ...
	    while (--pc >= 0)
	    {
	    	char f=this.charAt(tlim-1-to);
	    	char b=suffix.charAt(length-po);
	      if ( f != b)
	      {
	        return false;
	      }
